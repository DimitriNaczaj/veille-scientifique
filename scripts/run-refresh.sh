...
DATABASE_PATH=${DATABASE_PATH:-$VEILLE_ROOT/data/veille.sqlite}
REFRESH_LIMIT=${REFRESH_LIMIT:-200}

# Première réécriture d’entrées existantes : on garde une copie datée.
if [ -f "$DATABASE_PATH" ]; then
    BACKUP_PATH="$DATABASE_PATH.bak-$(date +%Y%m%d-%H%M%S)"
    cp -p "$DATABASE_PATH" "$BACKUP_PATH"
    printf 'Sauvegarde : %s\n' "$BACKUP_PATH"
fi

exec "$PYTHON_BIN" -m veille refresh-abstracts \
    --config "$CONFIG_PATH" \
